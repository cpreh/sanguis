...
public:
  ...
  void health_pair(sanguis::client::optional_health_pair const &);

  void experience(sanguis::client::exp);

  void level(sanguis::client::level, sanguis::client::exp_for_next_level);

  void add_weapon(sanguis::weapon_description const &);

  void remove_weapon(sanguis::is_primary_weapon);

  void magazine_remaining(sanguis::is_primary_weapon, sanguis::magazine_remaining);

  void reload_time(sanguis::is_primary_weapon, sanguis::duration);

  void world_name(sanguis::world_name const &);

  void player_name(sanguis::player_name const &);

  void update(sanguis::duration);

  void update_server(sanguis::client::slowed_duration);
...
private:
  using weapon_widget_unique_ptr = fcppt::unique_ptr<sanguis::client::gui::hud::weapon_widget>;

  using optional_weapon_widget_unique_ptr = fcppt::optional::object<weapon_widget_unique_ptr>;

  [[nodiscard]] optional_weapon_widget_unique_ptr &weapon_widget(sanguis::is_primary_weapon);

  [[nodiscard]] sanguis::client::gui::hud::weapon_widget &
      weapon_widget_checked(sanguis::is_primary_weapon);

  template <typename Function>
  void update_weapon_widgets(Function const &);

  template <typename Function>
  void foreach_weapon(Function const &);

  void update_exp();

  void create_details();

  void destroy_details();
  ...
  using weapon_details_unique_ptr = fcppt::unique_ptr<sanguis::client::gui::hud::weapon_details>;

  using optional_weapon_details_unique_ptr = fcppt::optional::object<weapon_details_unique_ptr>;
