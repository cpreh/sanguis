...
{
  FCPPT_NONMOVABLE(action_handler);
...
public:
  ...
  void handle_action(sanguis::client::control::actions::any const &);

  void handle_binary_action(sanguis::client::control::actions::binary const &);

  void handle_cursor_action(sanguis::client::control::actions::cursor const &);

  void handle_nullary_action(sanguis::client::control::actions::nullary const &);

  void handle_scale_action(sanguis::client::control::actions::scale const &);

  [[nodiscard]] sanguis::client::control::optional_cursor_position const &cursor_position() const;

private:
  void update_direction(
      fcppt::reference<sanguis::client::control::scalar>, sanguis::client::control::key_scale);

  void update_rotation();

  void handle_shooting(bool, sanguis::is_primary_weapon);

  void handle_drop(sanguis::is_primary_weapon);

  void handle_reload(sanguis::is_primary_weapon);

  void send_cheat(sanguis::cheat_type, sge::console::arg_list const &);

  [[nodiscard]] fcppt::signal::auto_connection cheat_connection(
      sge::console::object_ref,
      sanguis::cheat_type,
