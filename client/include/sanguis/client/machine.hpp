...
class machine : public boost::statechart::
                    state_machine<sanguis::client::machine, sanguis::client::states::menu>
{
...
public:
  ...
  void quickstart(alda::net::port);

  void connect(alda::net::host const &, alda::net::port);

  void disconnect();

  void send(sanguis::messages::client::base const &);

  [[nodiscard]] bool process(sanguis::duration const &);

  void quit();

  [[nodiscard]] sanguis::client::config::settings::object &settings();

  [[nodiscard]] sanguis::client::args::result const &options() const;

  [[nodiscard]] sge::gui::style::base const &gui_style() const;

  [[nodiscard]] sge::renderer::device::ffp &renderer() const;

  [[nodiscard]] sge::font::object &font_object() const;

  [[nodiscard]] sge::console::gfx::object &console_gfx();

  [[nodiscard]] sanguis::client::load::context const &resources() const;

  [[nodiscard]] sge::viewport::manager &viewport_manager() const;

  [[nodiscard]] sanguis::client::cursor &cursor_gfx() const;
...
private:
  ...
  void process_sge_event(awl::event::base const &);

  void connect_callback();

  void error_callback(fcppt::string const &, boost::system::error_code const &);
