...
class sounds
{
  FCPPT_NONMOVABLE(sounds);

public:
  sounds(sge::audio::loader_ref, sge::audio::player_ref);

  ~sounds();

  [[nodiscard]] sanguis::client::load::resource::optional_sound load(fcppt::string const &) const;

  [[nodiscard]] sanguis::client::load::resource::optional_sound
  load_path(std::filesystem::path const &) const;

private:
  using optional_buffer = fcppt::optional::object<sge::audio::buffer_unique_ptr>;

  using buffer_map = std::map<std::filesystem::path, optional_buffer>;
