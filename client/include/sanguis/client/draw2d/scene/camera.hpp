...
class camera
{
  FCPPT_NONMOVABLE(camera);

public:
  explicit camera(sge::renderer::device::core_ref);

  ~camera(); // NOLINT(performance-trivially-destructible)

  void
  update(sanguis::client::slowed_duration, sanguis::client::draw2d::optional_player_center const &);

  void reset_translation();

  [[nodiscard]] sanguis::client::draw2d::optional_translation translation() const;

  [[nodiscard]] sanguis::client::control::optional_attack_dest
  translate_attack_dest(sanguis::client::control::cursor_position const &) const;
