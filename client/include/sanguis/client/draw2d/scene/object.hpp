...
class object : public sanguis::client::draw::base
{
...
private:
  ...
  void update(
      ...
      sanguis::client::control::optional_cursor_position const &) override;

  void draw(sge::renderer::context::ffp &) override;

  void overlay(sge::renderer::context::ffp &) override;
  ...
  insert_own(sanguis::client::draw2d::entities::own_unique_ptr &&);
  ...
  [[nodiscard]] sanguis::client::draw2d::entities::base &entity(sanguis::entity_id) const;

  [[nodiscard]] sanguis::client::draw2d::optional_player_center player_center() const;

  void change_world(sanguis::client::world_parameters const &);

  [[nodiscard]] sge::renderer::screen_size screen_size() const;

  [[nodiscard]] sge::renderer::target::viewport viewport() const;

  [[nodiscard]] sanguis::client::draw2d::entities::load_parameters load_parameters();

  [[nodiscard]] sanguis::client::draw2d::insert_own_callback insert_own_callback();

  ALDA_CALL_FRIEND_DISPATCHER;

  using result_type = void;

  void operator()(sanguis::messages::server::add_aoe_projectile const &);

  void operator()(sanguis::messages::server::add_aura const &);

  void operator()(sanguis::messages::server::add_buff const &);

  void operator()(sanguis::messages::server::add_destructible const &);

  void operator()(sanguis::messages::server::add_doodad const &);

  void operator()(sanguis::messages::server::add_enemy const &);

  void operator()(sanguis::messages::server::add_friend const &);

  void operator()(sanguis::messages::server::add_own_player const &);

  void operator()(sanguis::messages::server::add_pickup const &);

  void operator()(sanguis::messages::server::add_player const &);

  void operator()(sanguis::messages::server::add_projectile const &);

  void operator()(sanguis::messages::server::add_weapon_pickup const &);

  void operator()(sanguis::messages::server::change_weapon const &);

  void operator()(sanguis::messages::server::change_world const &);

  void operator()(sanguis::messages::server::die const &);

  void operator()(sanguis::messages::server::give_weapon const &);

  void operator()(sanguis::messages::server::health const &);

  void operator()(sanguis::messages::server::level_up const &);

  void operator()(sanguis::messages::server::max_health const &);

  void operator()(sanguis::messages::server::move const &);

  void operator()(sanguis::messages::server::remove const &);

  void operator()(sanguis::messages::server::remove_buff const &);

  void operator()(sanguis::messages::server::remove_weapon const &);

  void operator()(sanguis::messages::server::rotate const &);

  void operator()(sanguis::messages::server::speed const &);

  void operator()(sanguis::messages::server::weapon_status const &);

  void process_default_msg(sanguis::messages::server::base const &);

  template <typename Message>
  void configure_new_object(sanguis::client::draw2d::entities::unique_ptr &&, Message const &);
  ...
  fcppt::unique_ptr<sanguis::client::draw2d::scene::camera> const camera_;

  sanguis::client::weapon_pair player_weapons_;

  using entity_map = std::map<sanguis::entity_id, sanguis::client::draw2d::entities::unique_ptr>;

  using own_entity_list = std::list<sanguis::client::draw2d::entities::own_unique_ptr>;

  entity_map entities_;

  own_entity_list own_entities_;

  fcppt::unique_ptr<sanguis::client::draw2d::scene::background> const background_;

  using optional_hover_unique_ptr =
      fcppt::optional::object<sanguis::client::draw2d::scene::hover::base_unique_ptr>;
