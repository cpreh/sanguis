...
class object : public sanguis::client::object_base
{
  FCPPT_NONMOVABLE(object);

public:
  object(fcppt::log::context_reference, sanguis::client::args::result const &);
...
private:
  ...
  void create_server(alda::net::port);

  [[nodiscard]] awl::main::exit_code quit_server();
  ...
  using frame_timer = sge::timer::absolute<sanguis::clock>;

  frame_timer frame_timer_;

  using server_unique_ptr = fcppt::unique_ptr<sanguis::client::server>;

  using optional_server_unique_ptr = fcppt::optional::object<server_unique_ptr>;

  optional_server_unique_ptr server_;

  using scoped_machine = fcppt::scoped_state_machine<sanguis::client::machine>;
