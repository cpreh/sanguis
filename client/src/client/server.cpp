...
sanguis::client::server::server(
    ...
    : log_{_log_context, sanguis::client::log_location(), fcppt::log::parameters_no_function(fcppt::log::name{FCPPT_TEXT("server")})},
      ...
      impl_(
          ...
          _port),
      mutex_(),
      server_thread_([this] { this->mainloop(); })
{
}

FCPPT_PP_POP_WARNING

void sanguis::client::server::quit()
{
  this->reset_running();
...
sanguis::client::server::~server() = default;

awl::main::exit_code sanguis::client::server::run()
{
  server_thread_.join();
...
void sanguis::client::server::mainloop()
{
  try
  {
    impl_.run();
  ...
  {
    FCPPT_LOG_ERROR(
        log_, fcppt::log::out << FCPPT_TEXT("Error in server thread: ") << _exception.string())
  }
  catch (std::exception const &_exception)
  {
    FCPPT_LOG_ERROR(
        log_, fcppt::log::out << FCPPT_TEXT("Error in server thread: ") << _exception.what())
  }
