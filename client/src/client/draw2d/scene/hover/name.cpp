...
sanguis::client::draw2d::scene::hover::name::name(
    ...
    : text_(
          _renderer,
          _font,
          sge::font::from_fcppt_string(_name.text()),
          sge::font::text_parameters(sge::font::align_h::variant(sge::font::align_h::left())),
          sge::font::vector(
              _center.get().x(),
              _center.get().y() - _radius.get() - _font->metrics().height().get()),
          _name.color(),
          sge::renderer::texture::emulate_srgb::no)
{
  // TODO(philipp): Improve sge::font to make this easier
  text_.pos(text_.pos() - (text_.rect().size() / fcppt::literal<sge::font::unit>(2)).get_unsafe());
}

sanguis::client::draw2d::scene::hover::name::name(name &&) noexcept = default;

sanguis::client::draw2d::scene::hover::name &
sanguis::client::draw2d::scene::hover::name::operator=(name &&) noexcept = default;

sanguis::client::draw2d::scene::hover::name::~name() = default;

void sanguis::client::draw2d::scene::hover::name::draw(sge::renderer::context::ffp &_render_context)
{
  text_.draw_advanced(
      _render_context, sge::font::draw::set_matrices(false), sge::font::draw::set_states(false));
}
