    ...
    sge::texture::part const &_texture)
{
  ...
  fcppt::optional::object<pos_type> const divided{
  ...
  coordinates const base(
      fcppt::math::vector::null<pos_type>(), FCPPT_ASSERT_OPTIONAL_ERROR(divided));

  fcppt::optional::object<pos_type> const rel_divided{
      fcppt::math::vector::structure_cast<pos_type, fcppt::cast::size_fun>(-_translation.get()) /
      dim};

  pos_type const rel(FCPPT_ASSERT_OPTIONAL_ERROR(rel_divided) * fcppt::tuple::get<1U>(base));
