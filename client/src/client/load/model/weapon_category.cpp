...
sanguis::client::load::model::weapon_category::weapon_category(
    ...
    : animations_(sanguis::client::load::model::make_animations(_log, _weapon_category, _param))
{
}

sanguis::client::load::model::weapon_category::~weapon_category() = default;

sanguis::client::load::model::animation const &
sanguis::client::load::model::weapon_category::operator[](
    sanguis::client::load::animation_type const _anim) const
{
  return fcppt::optional::maybe(
             fcppt::container::find_opt_mapped(animations_, _anim),
             [this, _anim]()
             {
               ...
               if (_anim == fallback)
               {
                 throw sanguis::exception{FCPPT_TEXT("Default animation missing in TODO")};
               }

               return fcppt::make_cref((*this)[fallback]);
             },
             [](fcppt::reference<sanguis::client::load::model::animation_unique_ptr const> const
                    _animation) { return fcppt::make_cref(*_animation.get()); })
      .get();
}
