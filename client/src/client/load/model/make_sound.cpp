...
sanguis::client::load::resource::optional_sound sanguis::client::load::model::make_sound(
    ...
    sanguis::client::load::resource::sounds_cref const _sounds)
{
  return fcppt::optional::bind(
      _opt_sound,
      [&_log, &_sounds](sanguis::model::animation_sound const &_sound)
      {
        sanguis::client::load::resource::optional_sound const result{_sounds->load(_sound.get())};

        if (!result.has_value())
        {
          FCPPT_LOG_ERROR(_log, fcppt::log::out << FCPPT_TEXT("Unable to load sound ") << _sound)
        }
