    ...
    sanguis::client::load::resource::texture_name_map &&_result)
{
  ...
  auto file(fcppt::filesystem::open_exn<fcppt::filesystem::ifstream, sanguis::exception>(
  ...
  while (std::getline(file, line))
  {
    ...
    fcppt::string::size_type const equal(line.find('='));

    if (equal == fcppt::string::npos)
    {
      FCPPT_LOG_WARNING(
          _log,
          fcppt::log::out << FCPPT_TEXT("Error in .id file \")")
                          << fcppt::filesystem::path_to_string(_path) << FCPPT_TEXT("\" in line ")
                          << line_num << FCPPT_TEXT('!'))

      continue;
    }

    _result[sanguis::client::load::resource::texture_identifier(line.substr(0, equal))] =
        std::filesystem::path{line.substr(equal + 1)};
  }
