      ...
      start_{[this]
             {
               FCPPT_LOG_DEBUG(
                   this->log_,
                   fcppt::log::out << FCPPT_TEXT("Trying to load settings from ")
                                   << fcppt::filesystem::path_to_string(this->path_))
               ...
               return fcppt::either::match(
                   ...
                   [](sge::parse::ini::start &&_start) -> sge::parse::ini::start
                   { return std::move(_start); });
             }()}
{
}
...
sanguis::client::config::settings::object::~object() = default;

sge::parse::ini::start &sanguis::client::config::settings::object::sections() { return start_; }

sge::parse::ini::start const &sanguis::client::config::settings::object::sections() const
{
  return start_;
}

void sanguis::client::config::settings::object::save() const
{
  FCPPT_LOG_DEBUG(
      log_,
      fcppt::log::out << FCPPT_TEXT("Trying to save settings to ")
                      << fcppt::filesystem::path_to_string(path_))

  if (!sge::parse::ini::output::to_file(path_, start_))
  {
    FCPPT_LOG_ERROR(log_, fcppt::log::out << FCPPT_TEXT("Writing the settings failed!"))
  }
}

fcppt::log::object &sanguis::client::config::settings::object::log() const { return log_; }
