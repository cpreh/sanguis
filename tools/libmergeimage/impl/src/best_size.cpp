...
{
  for (sge::image::size_type index(0);
       index <= sanguis::tools::libmergeimage::impl::max_exponent::value;
       ++index)
  {
    sge::image::size_type const size(sanguis::tools::libmergeimage::impl::pow2(index));

    if (sanguis::tools::libmergeimage::impl::cells_in_dim(size, _cell_size) >= _count)
    {
      return size;
    }
  }
