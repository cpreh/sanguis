...
class merge_result
{
  FCPPT_NONCOPYABLE(merge_result);

public:
  merge_result(sanguis::model::cell_size, sanguis::tools::libmergeimage::image_vector &&);

  SANGUIS_TOOLS_LIBMERGEIMAGE_SYMBOL
  merge_result(merge_result &&) noexcept;

  SANGUIS_TOOLS_LIBMERGEIMAGE_SYMBOL
  merge_result &operator=(merge_result &&) noexcept;

  SANGUIS_TOOLS_LIBMERGEIMAGE_SYMBOL
  ~merge_result();

  [[nodiscard]] SANGUIS_TOOLS_LIBMERGEIMAGE_SYMBOL sanguis::model::cell_size cell_size() const;
