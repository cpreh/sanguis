...
sanguis::model::animation sanguis::tools::animations::get_or_default_animation(
    ...
    sanguis::model::animation_name const &_animation_name)
{
  return fcppt::optional::from(
      fcppt::optional::bind(
          fcppt::container::find_opt_mapped(_model.parts(), _part_name),
          [&_weapon_category_name,
           &_animation_name](fcppt::reference<sanguis::model::part const> const _part)
          {
            return fcppt::optional::copy_value(fcppt::optional::bind(
                fcppt::container::find_opt_mapped(
                    _part.get().weapon_categories(), _weapon_category_name),
                [&_animation_name](
                    fcppt::reference<sanguis::model::weapon_category const> const _weapon_category)
                {
                  return fcppt::container::find_opt_mapped(
                      _weapon_category.get().animations(), _animation_name);
                }));
          }),
      []
      {
        return sanguis::model::animation(
            sanguis::model::animation_range(0U, 1U),
