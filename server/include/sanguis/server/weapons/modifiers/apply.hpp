...
Parameters apply(
    ...
    sanguis::server::weapons::modifiers::guaranteed<Parameters> const &_guaranteed_modifiers,
    sanguis::server::weapons::modifiers::potential<Parameters> const &_potential_modifiers,
    Parameters &&_parameters)
{
  ...
  auto const result(sanguis::server::random::draw<modifier_container>(
      ...
      sanguis::server::random::create_function<callback, callback>(
          fcppt::function<callback(callback const &)>{[](callback const &_callback)
                                                      { return _callback; }}),
      sanguis::server::random::less_function<callback>(
          fcppt::function<bool(callback const &, callback const &)>{
              [](callback const &_callback1, callback const &_callback2)
              { return std::less<callback>()(_callback1, _callback2); }}),
      sanguis::server::random::equal_function<callback>(
          fcppt::function<bool(callback const &, callback const &)>{
              [](callback const &_callback1, callback const &_callback2)
              { return _callback1 == _callback2; }})));

  for (auto const &modifier : _guaranteed_modifiers.get())
  {
    modifier(_random_generator, _difficulty, fcppt::make_ref(_parameters));
  }

  for (auto const &modifier : result)
  {
    modifier(_random_generator, _difficulty, fcppt::make_ref(_parameters));
  }

  return std::forward<Parameters>(_parameters);
}
