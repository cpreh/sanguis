...
sanguis::server::buffs::create_callback burn_create(
...
{
  static_assert(
      std::is_base_of_v<sanguis::server::buffs::burn, Buff>, "Buff must derive from buffs::slow");

  return sanguis::server::buffs::create_callback{
      [_interval, _damage, _damage_values](sanguis::server::entities::base_ref const _entity)
      {
        FCPPT_PP_PUSH_WARNING
        FCPPT_PP_DISABLE_GCC_WARNING(-Wattributes)
        return fcppt::optional::map(
            fcppt::cast::dynamic<sanguis::server::entities::with_health>(_entity.get()),
            [_interval, _damage, _damage_values](
                fcppt::reference<sanguis::server::entities::with_health> const _with_health)
            {
              return fcppt::unique_ptr_to_base<sanguis::server::buffs::buff>(
                  fcppt::make_unique_ptr<Buff>(
                      _with_health,
                      _interval,
                      _damage,
                      sanguis::server::damage::unmodified(_damage_values)));
            });
