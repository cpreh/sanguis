...
public:
  ...
  message_function(State &_state, sanguis::server::player_id const _id) : state_(_state), id_(_id)
  {
  }

  template <typename Message>
  result_type operator()(Message const &_message) const
  {
    return state_.get()(id_, _message);
  }

private:
  fcppt::reference<State> state_;
