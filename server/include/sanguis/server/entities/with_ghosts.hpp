...
class with_ghosts : public virtual sanguis::server::entities::base
{
  FCPPT_NONMOVABLE(with_ghosts);

public:
  with_ghosts();

  explicit with_ghosts(sanguis::server::collision::ghost_container &&);

  ~with_ghosts() override;

protected:
  [[nodiscard]] sanguis::collision::world::body_enter_container
  add_ghost(sanguis::server::collision::ghost &&);

  void init_ghosts(sanguis::server::collision::ghost_container &&);
