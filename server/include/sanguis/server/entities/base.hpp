...
public:
  /**
	...
	*/
  [[nodiscard]] sanguis::server::entities::optional_transfer_result transfer(
  ...
  virtual void tick(sanguis::duration const &);

  /**
	\brief Called when the entity is removed from the game
	*/
  virtual void remove_from_game();

  /**
	\brief Called before an entity is removed from a world
	*/
  [[nodiscard]] virtual sanguis::server::entities::remove_from_world_result remove_from_world();

  [[nodiscard]] sanguis::server::environment::optional_object_ref environment() const;

  [[nodiscard]] virtual sanguis::server::center center() const = 0;
