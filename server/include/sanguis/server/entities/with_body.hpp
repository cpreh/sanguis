...
class with_body : public virtual sanguis::server::entities::with_ghosts,
...
{
  FCPPT_NONMOVABLE(with_body);

public:
  with_body(sanguis::server::radius, sanguis::server::optional_mass const &);
  ...
  [[nodiscard]] sanguis::server::radius radius() const override;
