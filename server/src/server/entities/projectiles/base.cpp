...
sanguis::server::entities::projectiles::base::base(
    ...
    : sanguis::server::entities::with_id(_load_context.next_id()),
      ...
      sanguis::server::entities::with_velocity(
          _load_context.model_size(_model_path),
          sanguis::server::optional_mass(),
          sanguis::server::entities::movement_speed_initial(
              sanguis::server::entities::property::initial_max(_movement_speed.get())),
          _direction),
      team_(_team),
      life_timer_(
          sanguis::diff_timer::parameters(fcppt::make_cref(this->diff_clock()), _life_time.get()))
{
}
...
sanguis::server::team sanguis::server::entities::projectiles::base::team() const { return team_; }

bool sanguis::server::entities::projectiles::base::dead() const { return life_timer_.expired(); }

void sanguis::server::entities::projectiles::base::world_collision() { this->expire(); }

boost::logic::tribool sanguis::server::entities::projectiles::base::can_collide_with_body(
    sanguis::server::entities::with_body const &_other) const
{
  return !_other.dead() &&
         sanguis::server::entities::is_type<sanguis::server::entities::with_health>(_other);
}

void sanguis::server::entities::projectiles::base::collision_with_body(
    sanguis::server::entities::with_body &_other)
{
  if (!this->dead())
  {
    this->do_damage(dynamic_cast<sanguis::server::entities::with_health &>(_other));
  }
}
...
{
  return sanguis::server::entities::projectiles::collision_group(this->team());
}
