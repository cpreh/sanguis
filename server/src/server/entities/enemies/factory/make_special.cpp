...
{
  using split_array = sanguis::server::random::split_array<2>;

  split_array const amounts(sanguis::server::random::split(
      _random_generator.get(),
      sanguis::server::random::min(sanguis::server::random::amount(
          _is_unique.get()
              ? 3U // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
              : 1U)),
      sanguis::server::random::max(sanguis::server::random::amount(
          _is_unique.get()
  ...
  sanguis::server::entities::enemies::attribute_container const modifier_result(
      sanguis::server::entities::enemies::modifiers::apply(
          _random_generator.get(),
          fcppt::make_ref(_parameters),
          sanguis::server::entities::enemies::factory::make_modifiers(
              _random_generator.get(), fcppt::array::get<0U>(amounts))));
  ...
  return fcppt::unique_ptr_to_base<sanguis::server::entities::with_id>(
      fcppt::make_unique_ptr<sanguis::server::entities::enemies::special>(
          ...
          sanguis::server::entities::enemies::factory::make_skills(
              _random_generator.get(), fcppt::array::get<1U>(amounts)),
          _is_unique));
}
