...
sanguis::server::entities::player::player(
    ...
    : sanguis::server::entities::ifaces::with_team(),
      sanguis::server::entities::with_velocity(
          _load_context.model_size(sanguis::load::model::player_path()),
          sanguis::server::optional_mass(),
          sanguis::server::entities::movement_speed_initial(
              sanguis::server::entities::property::initial_zero(_speed.get())),
          sanguis::server::direction(0.F)),
      sanguis::server::entities::with_auras_id(fcppt::container::make<
                                               sanguis::server::auras::container>(
          fcppt::unique_ptr_to_base<
              sanguis::server::auras::aura>(fcppt::make_unique_ptr<
              ...
              sanguis::server::add_sight_callback(
                  ...
                  { this->add_sight_range(_entity, _created); }),
              sanguis::server::remove_sight_callback(
                  [this](sanguis::server::entities::with_id const &_entity)
                  { this->remove_sight_range(_entity); }))),
          fcppt::unique_ptr_to_base<sanguis::server::auras::aura>(
              fcppt::make_unique_ptr<sanguis::server::auras::weapon_pickup_candidates>(
                  // with_velocity needs to be initialized first!
                  this->radius(),
                  sanguis::server::add_weapon_pickup_callback{
                      [this](sanguis::server::entities::pickups::weapon_ref const _entity)
                      { this->weapon_pickup_add_candidate(_entity); }},
                  sanguis::server::remove_weapon_pickup_callback{
                      [this](sanguis::server::entities::pickups::weapon &_entity)
                      { this->weapon_pickup_remove_candidate(_entity); }})))),
      ...
      sanguis::server::entities::with_perks(_random_generator),
      sanguis::server::entities::with_weapon(
          sanguis::server::weapons::optional_unique_ptr(
              sanguis::server::weapons::player_start_weapon(_weapon_parameters)),
      ...
      level_(0U),
      skill_points_(0U),
      perk_tree_(sanguis::server::perks::tree::create()),
      desired_speed_(fcppt::math::vector::null<sanguis::server::speed::value_type>()),
      weapon_pickups_()
{
}
...
sanguis::server::entities::player::~player() = default;

void sanguis::server::entities::player::add_exp(sanguis::server::exp const _exp)
{
  ...
  sanguis::server::level const new_level{sanguis::server::level_from_exp(exp_)};
  ...
  skill_points_ += sanguis::server::skill_points(new_level.get() - old_level.get());
...
bool sanguis::server::entities::player::perk_choosable(sanguis::perk_type const _perk) const
{
  return skill_points_.get() > 0U &&
         sanguis::server::perks::tree::choosable(perk_tree_, _perk, level_);
}

void sanguis::server::entities::player::add_perk(sanguis::perk_type const _type)
{
  sanguis::server::perks::tree::choose(perk_tree_, _type);

  sanguis::server::entities::with_perks::add_perk(_type);

  --skill_points_;
}

void sanguis::server::entities::player::change_speed(sanguis::server::speed const &_speed)
{
    ...
    sanguis::is_primary_weapon const _is_primary)
{
  ...
  fcppt::optional::maybe_void(
      sanguis::server::closest_entity(
          *this,
          weapon_pickups_,
          [_is_primary](sanguis::server::entities::pickups::weapon const &_pickup)
          { return sanguis::weapon_type_to_is_primary(_pickup.weapon_type()) == _is_primary; }),
      [this](fcppt::reference<sanguis::server::entities::pickups::weapon> const _pickup)
      { this->pickup_weapon(_pickup.get().obtain()); });
...
sanguis::server::perks::tree::container const &sanguis::server::entities::player::perk_tree() const
{
  return perk_tree_;
}

sanguis::server::skill_points sanguis::server::entities::player::skill_points() const
{
  return skill_points_;
}

sanguis::server::player_id sanguis::server::entities::player::player_id() const
{
  return player_id_;
}

sanguis::server::level sanguis::server::entities::player::level() const { return level_; }

sanguis::server::team sanguis::server::entities::player::team() const
{
...
void sanguis::server::entities::player::update_speed()
{
  fcppt::optional::maybe(
      fcppt::math::vector::atan2(desired_speed_.get()),
      [this] { this->movement_speed().current(fcppt::literal<sanguis::server::space_unit>(0)); },
      [this](sanguis::server::space_unit const _dir)
      {
        this->direction(sanguis::server::direction(_dir));

        // TODO(philipp): don't set the speed to max!
        // Allow for controls that are not binary
        sanguis::server::entities::property::current_to_max(this->movement_speed());
      });
}

void sanguis::server::entities::player::add_sight_range(
    sanguis::server::entities::with_id const &_entity,
    sanguis::collision::world::created const _created)
{
  fcppt::optional::maybe_void(
...
sanguis::messages::server::unique_ptr sanguis::server::entities::player::add_message(
    sanguis::server::player_id const _receiver,
    sanguis::collision::world::created const _created) const
{
  return _receiver == this->player_id()
             ? this->add_message_impl<sanguis::messages::server::add_own_player>(_created)
             : this->add_message_impl<sanguis::messages::server::add_player>(_created);
}

template <typename Message>
sanguis::messages::server::unique_ptr sanguis::server::entities::player::add_message_impl(
    sanguis::collision::world::created const _created) const
{
  return sanguis::messages::server::create_ptr(alda::message::init_record<Message>(
      ...
      sanguis::messages::roles::name{} = sge::charconv::fcppt_string_to_utf8(this->name().get())));
}
...
{
  return sanguis::collision::world::body_group::player;
}
