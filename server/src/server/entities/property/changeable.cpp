...
sanguis::server::entities::property::changeable::changeable(
    ...
    : sanguis::server::entities::property::base(_initial.get_base()),
      current_(_initial.get_current()),
      max_(_initial.get_base()),
      change_signal_(),
      max_change_signal_()
{
  this->check_current();
...
sanguis::server::entities::property::changeable::~changeable() = default;

void sanguis::server::entities::property::changeable::current(
    sanguis::server::entities::property::value const _current)
{
  sanguis::server::entities::property::diff const diff(_current - this->current());
  ...
  if (std::abs(diff.get()) >
      fcppt::literal<sanguis::server::entities::property::value>(
...
sanguis::server::entities::property::changeable::register_max_change_callback(
    sanguis::server::entities::property::change_callback &&_callback)
{
  return max_change_signal_.connect(std::move(_callback));
}

void sanguis::server::entities::property::changeable::on_recalc_max(
    sanguis::server::entities::property::value const _max)
{
  sanguis::server::entities::property::diff const diff(_max - max_);

  max_ = _max;

  max_change_signal_(sanguis::server::entities::property::change_event(diff));

  this->current(std::min(current_, max_));
}
