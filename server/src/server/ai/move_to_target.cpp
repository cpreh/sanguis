...
void sanguis::server::ai::move_to_target(
...
{
  sanguis::server::entities::property::changeable &speed(_me.movement_speed());

  speed.current(fcppt::optional::maybe(
      _opt_angle,
      fcppt::const_(fcppt::literal<sanguis::server::space_unit>(0)),
      [_speed_factor, _target, &_me, &speed](sanguis::server::angle const _angle)
      {
        _me.direction(sanguis::server::direction(_angle.get()));
        ...
        return std::min(
            fcppt::literal<sanguis::server::space_unit>(1.3F) *
                sanguis::server::collision::distance_entity_pos(_me, _target.get().get()) /
                sanguis::server::ai::update_interval().count(),
            speed.max() * _speed_factor.get());
      }));
}
