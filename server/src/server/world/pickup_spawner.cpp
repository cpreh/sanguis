...
sanguis::server::world::pickup_spawner::pickup_spawner(
    ...
    : random_generator_(_random_generator),
      ...
      env_(_env),
      distributor_(sanguis::server::world::pickup_spawner::distributor::vector{
          std::make_pair(
              ...
              sanguis::server::world::pickup_spawner::spawn_function(
                  ...
                  { this->spawn_health(_center, _difficulty); })),
          std::make_pair(
              sanguis::server::pickup_probability(1.F),
              this->make_spawn_weapon(sanguis::weapon_type(sanguis::primary_weapon_type::pistol))),
          std::make_pair(
              sanguis::server::pickup_probability(
                  0.8F // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
                  ),
              this->make_spawn_weapon(sanguis::weapon_type(sanguis::primary_weapon_type::shotgun))),
          std::make_pair(
              sanguis::server::pickup_probability(1.F),
      ...
      spawn_prob_(real_distribution(
          real_distribution::param_type::min(0.F), real_distribution::param_type::sup(1.F)))
{
}

FCPPT_PP_POP_WARNING

sanguis::server::world::pickup_spawner::~pickup_spawner() = default;

void sanguis::server::world::pickup_spawner::spawn(
    sanguis::server::pickup_probability const _prob,
    sanguis::server::center const &_center,
    sanguis::server::entities::enemies::difficulty const _difficulty)
{
  if (spawn_prob_(random_generator_.get()) > _prob.get())
  {
    return;
  }

  distributor_.execute(random_generator_.get())(_center, _difficulty);
}

void sanguis::server::world::pickup_spawner::spawn_health(
    sanguis::server::center const &_center,
    sanguis::server::entities::enemies::difficulty const _difficulty)
{
  this->spawn_entity(
      fcppt::unique_ptr_to_base<sanguis::server::entities::with_id>(
          fcppt::make_unique_ptr<sanguis::server::entities::pickups::health>(
              env_->load_context(),
              sanguis::server::team::players,
              sanguis::server::health(
                  10.F // NOLINT(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
                  * std::sqrt(_difficulty.get())))),
      _center);
}

void sanguis::server::world::pickup_spawner::spawn_weapon(
    sanguis::server::center const &_center,
    sanguis::server::entities::enemies::difficulty const _difficulty,
  ...
  this->spawn_entity(
      fcppt::unique_ptr_to_base<sanguis::server::entities::with_id>(
          fcppt::make_unique_ptr<sanguis::server::entities::pickups::weapon>(
              env_->load_context(),
              sanguis::server::team::players,
              sanguis::server::weapons::create(weapon_parameters_, _weapon_type, _difficulty))),
      _center);
}

void sanguis::server::world::pickup_spawner::spawn_entity(
    sanguis::server::entities::with_id_unique_ptr &&_entity, sanguis::server::center const &_center)
{
  sanguis::server::environment::insert_no_result(
      env_.get(), std::move(_entity), sanguis::server::entities::insert_parameters_center(_center));
}

sanguis::server::world::pickup_spawner::spawn_function
sanguis::server::world::pickup_spawner::make_spawn_weapon(sanguis::weapon_type const &_weapon_type)
{
  return sanguis::server::world::pickup_spawner::spawn_function(
      [this, _weapon_type](
          sanguis::server::center const &_center,
          sanguis::server::entities::enemies::difficulty const _difficulty)
      { this->spawn_weapon(_center, _difficulty, _weapon_type); });
}
