...
void rect(sanguis::creator::rect const &_rect, Callback const &_callback)
{
  using int_type = sanguis::creator::pos::value_type;
  ...
  {
    _callback(sanguis::creator::pos(x0, y));
    _callback(sanguis::creator::pos(x1 - 1, y));
  }

  for (int_type x = x0 + 1; x < x1; ++x)
  {
    _callback(sanguis::creator::pos(x, y0));
    _callback(sanguis::creator::pos(x, y1 - 1));
  }
}
