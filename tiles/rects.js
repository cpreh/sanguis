...
(function() {
  ...
  draw_corridor = function(ctx, tilesize, p1, p2) {
    var dx, dy, set_tile, sgnx, sgny, sign;
    set_tile = function(x, y, color) {
      ctx.fillStyle = color != null ? color : '#883';
      return ctx.fillRect(tilesize * x, tilesize * y, tilesize, tilesize);
    };
    ...
    if (Math.abs(dy) > Math.abs(dx)) {
      if (dx !== 0) {
        ctx.fillStyle = '#088';
        ctx.fillRect(tilesize * p1.x, tilesize * (p1.y - 1), tilesize * (p2.x - p1.x + 1.5 * sgnx + 0.5), tilesize * 3.);
      }
      ctx.fillStyle = '#800';
      ctx.fillRect(tilesize * (p2.x - 1), tilesize * p1.y, tilesize * 3., tilesize * (p2.y - p1.y + 1));
      ctx.fillStyle = '#ff0';
      ctx.fillRect(tilesize * p1.x, tilesize * p1.y, tilesize * (p2.x - p1.x), tilesize * 1.);
      ctx.fillRect(tilesize * p2.x, tilesize * p1.y, tilesize * 1., tilesize * (p2.y - p1.y));
    } else {
      if (dy !== 0) {
        ctx.fillStyle = '#808';
        ctx.fillRect(tilesize * (p1.x - 1), tilesize * p1.y, tilesize * 3., tilesize * (p2.y - p1.y + 1.5 * sgny + 0.5));
      }
      ctx.fillStyle = '#080';
      ctx.fillRect(tilesize * p1.x, tilesize * (p2.y - 1), tilesize * (p2.x - p1.x + 1), tilesize * 3.);
      ctx.fillStyle = '#ff0';
      ctx.fillRect(tilesize * p1.x, tilesize * p1.y, tilesize * 1., tilesize * (p2.y - p1.y));
      ctx.fillRect(tilesize * p1.x, tilesize * p2.y, tilesize * (p2.x - p1.x), tilesize * 1.);
    }
    set_tile(p1.x, p1.y, '#0ff');
    return set_tile(p2.x, p2.y, '#0ff');
  };

  init = function() {
    var canvas, ctx, highlight_rect, mouse_pos, rects, redraw, tilesize;
    ...
    redraw = function(event) {
      clear(ctx);
      return draw_corridor(ctx, tilesize, new Pos(25, 25), mouse_pos(event));
    };
    ...
    return canvas.addEventListener('mousemove', redraw);
  };
