...
sanguis::tiles::impl::neighbors_to_string(sanguis::tiles::impl::neighbors<Tile> const &_tiles)
{
  return fcppt::string{'('} +
         fcppt::algorithm::join_strings(
             fcppt::algorithm::map<std::vector<fcppt::string>>(
                 _tiles, [](Tile const _tile) { return sanguis::tiles::impl::to_string(_tile); }),
             FCPPT_TEXT(",")) +
         fcppt::string{')'};
}

#define SANGUIS_TILES_INSTANTIATE_NEIGHBORS_TO_STRING(tile_type) \
...
SANGUIS_CREATOR_INSTANTIATE_TILE(SANGUIS_TILES_INSTANTIATE_NEIGHBORS_TO_STRING);
